...
from tools import obj_init_config, send_request, verify_init_value_isexist
...
class generator():
    ...
    def pv_instance(self, device_no, sleep_time):
        str_url = obj_init_config.get("api_info", "pv_url").format(device_no, obj_init_config.get("device_epc", "pv_instance"))
        while(True):
    ...
    def battery_instance(self, device_no, sleep_time):
        str_url = obj_init_config.get("api_info", "battery_url").format(device_no, obj_init_config.get("device_epc", "battery_elec_3"))
        while(True):
    ...
    def fc_instance(self, device_no, sleep_time):
        str_url = obj_init_config.get("api_info", "fc_url").format(device_no, obj_init_config.get("device_epc", "fc_instance"))
        while(True):
            str_value = str(random.randrange(0, 1000))
            str_data = str_value.zfill(4)
            send_request(str_url, str_data)
    ...
    def accumulate(self, device_type, device_no, scope, sleep_time):
        epc = obj_init_config.get("device_epc", scope)
        str_scope = f"""{device_no}_{obj_init_config.get('device_epc', scope)}"""
        verify_init_value_isexist(device_type, str_scope)
        config_value = device_type + "_url"
        str_url = obj_init_config.get("api_info", config_value).format(device_no, epc)
        str_device_scope = device_no + "_" + epc
        int_start_data = int(obj_init_config.get(device_type, str_device_scope))
        ...
        while(True):
            ...
            if int_now_hour >= start_time and int_now_hour < end_time:
                ...
                str_data = str(obj_init_config.get(device_type, str_device_scope))
                str_format_data = str_data.zfill(8)
            send_request(str_url, str_format_data)
            obj_init_config.set(device_type, str_device_scope, str(int_data))
            print(f"""Sleep for {sleep_time} secs.""")
            time.sleep(int(sleep_time))

if __name__ == '__main__':
    job = generator()
    obj_init_config.set("pv", "11", "22")
    pass
